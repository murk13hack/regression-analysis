...
class basest:
    def __init__(self, x, y):
        ...
        self.F_critical = stats.f.ppf(1 - self.alpha, self.m, self.df_ost)
    ...
    @property 
    def comp_of_var(self):
        if self.comp_of_var_ is None:
            Ff = np.var(self.x, ddof=1)/np.var(self.y, ddof=1)
            Fcr = self.F_critical
            p = stats.f.cdf(Ff, self.m, self.df_ost)
            self.comp_of_var_ = {
                "F-fact": Ff,
                "F-critical": Fcr,
                "p": p
            }
        return self.comp_of_var_
    
    @property
    def comp_sample_averages(self): # T-статистика
        if self.comp_sample_averages_ is None:
            Tf = stats.ttest_ind(self.x, self.y, equal_var=True)
            Tcrit = stats.t.ppf(1 - self.alpha / 2, self.m)
            p = Tf.pvalue
            self.comp_sample_averages_ = {
                "T-fact": Tf,
                "T-critical": Tcrit,
                "p": p
            }
        return self.comp_sample_averages_
    
    @property
    def var_ratio(self): # коэффициент вариации
        if self.var_ratio_ is None:
            TSS = np.sum((self.y - self.y_mean)**2) # общая вариация
            ESS = np.sum((self.y_pred - self.y_mean)**2) # объясненная вариация
            RSS = np.sum((self.y - self.y_pred)**2) # остаточная вариация
            self.var_ratio_ = {
                "general variation": TSS,
                "explained variation": ESS,
                "residual variation": RSS
            }
        return self.var_ratio_
    ...
    def scheme_of_variance_analysis(self):
        if self.scheme_of_variance_analysis_ is None: 
            ...
            data = [["regression", self.m, self.SS_fact, self.MS_fact, F, self.F_critical, F > self.F_critical],
            ["remains", self.df_ost, self.SS_ost, self.MS_ost, "", "", ""],
            ["total", self.m+self.df_ost, self.SS_fact+self.SS_ost, "", "", "", f"p_value: {p_value}"]]
            self.scheme_of_variance_analysis_ = tabulate(pd.DataFrame(data, columns=["Sources of variation", "DF", "SS", "MS", "F - criteria", "F - critical", "F - criteria > F - critical"]), headers="keys", tablefmt="grid")
    ...
    def T_ratio_regr(self):
        if self.T_ratio_regr_ is None:
            ...
            tcrit = self.comp_sample_averages["T-critical"]
    ...
    def y_pred_plus_5(self):
        if self.y_pred_plus_5_ is None:
            """Прогнозное значение результата при увеличении фактора на 5% от среднего уровня."""
    ...
    def individual_confidence_intervals(self):
        if self.individual_confidence_intervals_ is None:
            ...
            se_ind = sqrt(self.MS_ost * (1 + 1/self.n + (self.x - np.mean(self.x))**2 / self.SS_x))
            t_crit = self.comp_sample_averages["T-critical"]
    ...
    def goldfeld_quandt_test(self):
        ...
        x1 = self.x[sorted_indices[:split]].reshape(-1, 1)
        x2 = self.x[sorted_indices[-split:]].reshape(-1, 1)
    ...
    def print_summary(self):
            ...
            print("\nСравнение дисперсий:")
            print(tabulate(pd.DataFrame([self.comp_of_var]), headers="keys", tablefmt="grid"))

            print("\nСравнение средних (t-тест):")
            print(tabulate(pd.DataFrame([self.comp_sample_averages]), headers="keys", tablefmt="grid"))

            print("\nВариация:")
            print(tabulate(pd.DataFrame([self.var_ratio]), headers="keys", tablefmt="grid"))
